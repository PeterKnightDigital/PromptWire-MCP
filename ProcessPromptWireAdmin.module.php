...
/**
 * PromptWire Admin - Sync Status Dashboard
 * 
...
class ProcessPromptWireAdmin extends Process {
    ...
    public static function getModuleInfo(): array {
        return [
            'title' => 'PromptWire Admin',
            'summary' => 'Admin UI for PromptWire: manage content sync between ProcessWire and YAML files',
            'version' => '1.1.0',
            'author' => 'Peter Knight',
            'icon' => 'refresh',
            'requires' => ['ProcessWire>=3.0.165', 'PromptWire'],
            'permission' => 'promptwire-admin',
            'permissions' => [
                'promptwire-admin' => 'Use PromptWire Admin dashboard',
            ],
            'page' => [
                'name' => 'promptwire-admin',
                'parent' => 'setup',
                'title' => 'PromptWire Admin',
            ],
        ];
    ...
    public function init() {
        ...
        // Path to PromptWire module's SyncManager
        $this->syncManagerPath = __DIR__ . '/src/Sync/SyncManager.php';
    }

    /**
     * Get Lucide icon SVG markup.
     * Delegates to the shared LucideIcons helper so icon paths are
     * maintained in one place across all PromptWire modules.
     * 
    ...
    protected function lucideIcon(string $name, int $size = 16): string {
        require_once __DIR__ . '/src/UI/LucideIcons.php';
        return \PromptWire\UI\LucideIcons::render($name, $size);
    }

    /**
     * Get SyncManager instance
     * 
     * @return \PromptWire\Sync\SyncManager
     */
    protected function getSyncManager() {
        require_once($this->syncManagerPath);
        return new \PromptWire\Sync\SyncManager($this->wire());
    }
    ...
    public function ___execute(): string {
        ...
        // Set page headline
        $this->headline($this->_('PromptWire Admin'));
        $this->browserTitle($this->_('PromptWire Admin'));
