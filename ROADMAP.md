...
- **`page.yaml` and `page.json` get a self-describing header.** `page.yaml` renders a comment block at the very top — page title, path, template, status, this site's pageId, and a one-line asset summary built from the same snapshot embedded in `page.meta.json`. `page.json` gets the equivalent as a top-level `_meta` object. Pure ergonomics for the operator opening a pulled file in an editor (or handing it to an AI agent that benefits from concrete context). Comments are stripped by every YAML parser in use (Symfony YAML, the in-tree simple parser, js-yaml on the MCP side); JSON readers ignore unrecognised top-level keys. Verified end-to-end against the existing push pipeline. The push-back code path also refreshes both the header AND the embedded snapshot after a successful save so subsequent drift checks see the current state of the page, not the pre-push baseline.
