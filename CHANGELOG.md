...
- **Fixed: the CLI `--help` banner reports the real module version.** `CommandRouter::help()` carried a hardcoded `'1.12.7'`, so a 1.13.0 install announced the wrong version to anyone reading `--help` (and to any agent that trusted it). It now reads the version from `getModuleInfo('PromptWire')`, returning `unknown` rather than a stale guess if the lookup fails. Same class of bug as the MCP server's hardcoded `serverInfo.version`, fixed in 81acaa1.
- **Module + MCP server version bumped to 1.13.1.**
...
- **Improved: YAML parse errors in `parsePageYaml` now include an actionable hint.** When js-yaml throws on a colon-in-value (e.g. `title: Foo: Bar` — a colon not inside a quoted string), the error message looked like `bad indentation of a mapping entry (70:25)` — correct but opaque. PromptWire now detects the `bad indentation` / `mapping entry` pattern and appends: `— likely cause: an unquoted colon in a field value (e.g. title: Foo: Bar must be title: "Foo: Bar")`. PHP's `yamlValue()` already quotes colons on export; this improves the experience for hand-edited or migrated YAML files that don't go through the exporter.
...
- **Default-changed: `pw_site_sync` no longer pushes `site/modules/*` unless asked.** Every sync run now prepends `site/modules/*` to the effective `excludeFilePatterns` and skips module code in the file step. To ship a plugin update alongside templates/content, pass the new `pushModules: true` flag (additive — your existing `excludeFilePatterns` are preserved either way). Rationale: the module tree is the easiest footgun in cross-environment deploys (accidental overwrite of a hotfix applied directly on prod, push of a half-finished plugin update), and the overwhelmingly common deploy is templates + pages + content. Dry-run plans now surface a `files.modulesExcluded` block when the auto-exclusion is in effect, so it's never silent.
- **Migration impact: breaking for the rare module-deploy workflow.** Saved prompts and scripts that relied on `pw_site_sync` shipping module updates by default will silently stop pushing them. The fix is a one-flag change: add `pushModules: true` to those calls. The `files.modulesExcluded` hint in the dry-run output makes the cause visible the first time someone hits it. `pw_site_compare` is unchanged — it still shows module diffs by default, which is the right behaviour for a diagnostic tool. Only `pw_site_sync` (the action tool) defaults are stricter.
- **Fixed: `pw_health.writesEnabled` is no longer a hardcoded lie.** The field has been a literal `false` since v1.0 — a leftover from the read-only phase that never got wired up. Operators reasonably read it as "writes are blocked on this server" and chase that down before discovering writes were never gated by it. Now computed from the actual gate state: CLI invocations report `true` with reason `cli`; web invocations (which can only reach `health()` after `promptwire-api.php` has validated the API key) report `true` with reason `api-key`, plus `+ip-allowlist` when `PROMPTWIRE_ALLOWED_IPS` is set and `+http` when `PROMPTWIRE_ALLOW_HTTP` is on. A new sibling `writesEnabledReason` field carries the breakdown so the *why* is visible without trial and error. The `no-api-key` reason exists for the defensive case where CommandRouter is loaded via a misconfigured web entry that never ran the auth check.
- **Migration impact: technically breaking for anything scripting against `writesEnabled === false`.** That value is now `true` on every correctly-configured install, which is the value those callers should have seen all along. The `writesEnabledReason` field is strictly additive and can be ignored.
- **Module + MCP server version bumped to 1.12.0.**

## 1.11.1 (1 May 2026)

- **New:** Frontend-usage scan on `remove` ops. Before `pw_template_fields_push` removes a field, it scans `site/templates/` for hardcoded references to the field name (`.php`, `.module`, `.inc`, `.html`) and emits a warning (scope: `frontend-usage`) listing file paths, line numbers, and snippets when any match. Complements v1.11.0's module-ownership awareness — that catches fields belonging to known modules; this catches the broader class of break where a regular field like `your_email` or `feedback` is hardcoded in a custom contact form's PHP (`$page->your_email = $input->post->your_email`). Removing the field leaves ProcessWire healthy but silently no-ops the assignment, so the form stops working with no admin-side error. Validated end-to-end on peterknight.digital: removing `blog_date` from `blog_post` now surfaces 6 real references across 5 blog listing templates that would have silently broken `sort=-blog_date` queries.
- **Implementation notes.** Fail-open on every error path (missing templates dir, FS permission, iterator exception) — the scan never blocks the classifier. Capped at 200 files / 2.0s wallclock, with up to 10 references listed per field and a summary of additional occurrences. Word-boundary regex (`\bfieldname\b`) keeps substring false-positives out (`blog_` vs `blog_date`). Warning, not danger, because scan false positives do exist (comments, string literals, similarly-named variables) and the removal may be a legitimate retirement cleanup — the operator gets full visibility without being blocked. Scan is skipped when the op is already a danger (e.g. core-flag removal of `title`) to keep the response uncluttered.
- **Shared-helper readiness.** `templateFieldsPushFrontendUsageWarnings(string $fieldName)` is a private method on `CommandRouter` that takes just a field name and returns `null` or a warning payload. v1.12's `pw_field_push` — which will delete field *definitions* entirely, a bigger blast radius than fieldgroup removal — will call the same helper in *blocker* mode.
- **Migration impact: zero.** Strictly additive on the `conflicts.warning[]` array. No existing tool's signature or output changes. Callers that handle warnings generically see one extra entry when a remove op has frontend-template references; callers that filter on `scope` can match on `'frontend-usage'` explicitly.
- **Module + MCP server version bumped to 1.11.1.** No other changes.
