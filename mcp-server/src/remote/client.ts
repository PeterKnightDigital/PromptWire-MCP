/**
 * PW-MCP Remote HTTP Client
 *
 * Makes authenticated HTTP requests to a remote PW-MCP API endpoint.
...
/**
 * Run a PW-MCP command on a remote site via HTTP
 *
 ...
 *
 * Required env vars:
 *   PW_REMOTE_URL - Full URL to pw-mcp-api.php on the remote site
    ...
    if (!response.ok) {
      switch (response.status) {
        ...
        case 404:
          return {
            success: false,
            error: `Remote API not found at ${remoteUrl} — ensure pw-mcp-api.php is deployed and accessible`,
          };
        case 500:
          return {
            success: false,
            error: 'Remote server error — check that PwMcp module is installed on the remote site',
          };
        default:
          return {
            success: false,
